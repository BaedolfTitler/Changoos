...
-- Soft searches for an inventory of the name `inv_name` and returns its wrapped peripheral
local function softFindPeripheral(inv_name)
    local names = peripheral.getNames()
    for k, nm in names do
        if str.softMatch(nm, inv_name) == inv_name then
...
local function findFluid(tankSet, fluidName)
    for tank_number, fluid in pairs(tankSet) do
