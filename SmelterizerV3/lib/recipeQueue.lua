...
local function recipeQueue()
    ...
    rq.resolveItems = function(smelterStack, recipe)
        ...
        local resRecipe = {}
            ...
            local t = false -- switch for avoiding spamming the user
            local foundItem
            if smelterStack.ItemCrafting then
                repeat
                    local fitem, itemSlot = inv.findItem(smelterStack.inventory, recipe.first)
                    foundItem = fitem -- rough way of moving the bundled return of the findItem function out of this scope
